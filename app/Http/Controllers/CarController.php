...
class CarController extends Controller
{
    ...
    {
        $cars = DB::table('cars')
            ->join('categories', 'cars.category_id', '=', 'categories.id')->select('cars.*', 'categories.name as category')->get();
        return response()->json($cars);
    }
    public function show($id)
    {
        $car = DB::table('cars')
            ->join('categories', 'cars.category_id', '=', 'categories.id')->select('cars.*', 'categories.name as category')->where('cars.id', $id)
            ->first();
        if (! $car) {
    ...
    public function store(Request $request)
    {
        ...
        return response()->json(['message' => 'Data mobil berhasil ditambah!', 'id' => $id], 201);
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['message' => 'Mobil berhasil di hapus!']);
    }
}
